...
public class Jellydew : ModNPC
{
    ...
    public override void HitEffect(NPC.HitInfo hit)
    {
        ...
        {
            for (int i = 0; i < 6; ++i)
                Dust.NewDust(NPC.Center, 26, 18, DustID.Water, Main.rand.NextFloat(-3, 3), Main.rand.NextFloat(-3, 3));
        }
    }
    ...
    public override float SpawnChance(NPCSpawnInfo spawnInfo)
    {
        ...
        {
            float chance = spawnInfo.Player.GetModPlayer<VerdantPlayer>().ZoneVerdant ? (spawnInfo.Water ? 0.4f : 0.05f) : 0f;
