...
class ApotheosisHand : ModProjectile, IDrawAdditive
{
    ...
    public void DrawAdditive(AdditiveLayer layer)
    {
        ...
        Main.spriteBatch.Draw(tex, drawPos, null, col, 0f, tex.Size() / 2f, Projectile.scale, SpriteEffects.None, 1f);
        Main.spriteBatch.Draw(tex, drawPos, null, col * 0.5f, 0f, tex.Size() / 2f, 1.5f * Projectile.scale, SpriteEffects.None, 1f);
        Main.spriteBatch.Draw(tex, drawPos, null, col * 0.25f, 0f, tex.Size() / 2f, 2f * Projectile.scale, SpriteEffects.None, 1f);
        ...
        for (int i = 0; i < 4; ++i)
        {
            Main.spriteBatch.Draw(tex, drawPos - new Vector2(10, 0), null, col * (1f / i), -rot + MathHelper.PiOver4, tex.Size(), 1 + ((i - 1) * 0.25f) * Projectile.scale, SpriteEffects.None, 1f);
            Main.spriteBatch.Draw(tex, drawPos + new Vector2(12, 0), null, col * (1f / i), rot - MathHelper.PiOver4, new(0, tex.Height), 1 + ((i - 1) * 0.25f) * Projectile.scale, SpriteEffects.FlipHorizontally, 1f);
        }
    }
}
