    ...
    class VerdantHealingMinion : ModProjectile, IDrawAdditive
    {
                ...
                if (Timer % 210 == 120 && p.active && !p.dead && Vector2.Distance(p.MountedCenter, projectile.Center - off) < rad * radMult * 196)
                {
                    if (p.statLife < p.statLifeMax2 - 10)
                    {
                        ...
                        for (int j = 0; j < 3; ++j)
                        {
                            ...
                            Projectile.NewProjectile(projectile.Center, particleVel, ModContent.ProjectileType<Misc.HealingParticle>(), 0, 0, projectile.owner, 0, 0);
                        }
                    }
                }
            }
        ...
        private void DrawCircle(SpriteBatch spriteBatch, Color lightColor, float scale, out Vector2 off)
        {
            ...
            if (alphaScale > 0.95f)
            {
                ...
                spriteBatch.Draw(circle, projectile.Center - Main.screenPosition - off, circle.Frame(), col, Timer * 0.006f, circle.Bounds.Center.ToVector2(), sc * scale * 1.04f, SpriteEffects.None, 1f);
                spriteBatch.Draw(circle, projectile.Center - Main.screenPosition - off, circle.Frame(), col, Timer * 0.006f, circle.Bounds.Center.ToVector2(), sc * scale * 0.96f, SpriteEffects.None, 1f);
            }
        }
