...
public class VerdantGrassLeaves : ModTile, IVerdantGrassTile
{
    ...
    public override void Unload() => CountsAsVerdantGrass = [];
    ...
    private static bool SpawnSpecial(int i, int j)
    {
        if (Main.rand.NextBool(250) && TearRainSystem.AnyRainingAt(j) && TileHelper.ValidBottom(i, j) && TileHelper.ValidBottom(i + 1, j) && !WorldGen.SolidTile(i, j + 1))
        {
            WorldGen.PlaceObject(i, j + 3, ModContent.TileType<TearPlant>(), true);
            return true;
        }

        if (Main.rand.NextBool(350) && TearRainSystem.AnyRainingAt(j) && TileHelper.ValidBottom(i, j) && !WorldGen.SolidTile(i, j + 1))
        {
            WorldGen.PlaceObject(i - 1, j + 1, ModContent.TileType<WaterPlant>(), true);
            return true;
        }
    ...
    private static bool NormalGrowth(int i, int j)
    {
        ...
        //bouncebloom
        bool baseBouncebloom = NPC.downedBoss1 && Main.rand.NextBool(150);
        if (baseBouncebloom && TileHelper.ValidTop(i, j) && TileHelper.ValidTop(i - 1, j) && TileHelper.ValidTop(i + 1, j) && Helper.AreaClear(i - 1, j - 2, 3, 2))
        {
        ...
        //blue stargazer
        if (!Main.dayTime && Main.rand.NextBool(60) && TileHelper.ValidTop(self) && TileHelper.ValidTop(i + 1, j) && Helper.AreaClear(i, j - 2, 2, 2))
        {
            WorldGen.PlaceTile(i, j - 2, ModContent.TileType<StargazerPlant>(), true, false, -1, Main.rand.Next(3));
            if (Main.netMode == NetmodeID.Server)
                NetMessage.SendTileSquare(-1, i, j - 1, 5, TileChangeType.None);
            return true;
        }
